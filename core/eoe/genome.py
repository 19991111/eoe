...
if TYPE_CHECKING:
    from .node import Node, NodeType, node_to_dict, node_from_dict
    from .agent import Agent

# 运行时导入 node_to_dict, node_from_dict
from .node import node_to_dict, node_from_dict

# 运行时仍然需要 Node 和 NodeType
...
class OperatorGenome:
    ...
    def __init__(self, innovation_manager: Optional[InnovationManager] = None, config=None):
    ...
    def add_edge(
        ...
        edge = {
            ...
            "is_modulatory": False,  # v17.0: 是否为调节性边 (门控用)
        }
    ...
    def forward(self, sensor_inputs: np.ndarray) -> np.ndarray:
        ...
        v13_sensor_nodes = {}
        v13_sensor_order = [
            NodeType.SENSE_EPF_CENTER,
            NodeType.SENSE_EPF_GRAD_X,
            NodeType.SENSE_EPF_GRAD_Y,
            NodeType.SENSE_KIF_CENTER,
            NodeType.SENSE_KIF_GRAD_X,
            NodeType.SENSE_KIF_GRAD_Y,
            NodeType.SENSE_ISF_CENTER,
            NodeType.SENSE_ISF_GRAD_X,
            NodeType.SENSE_ISF_GRAD_Y,
            NodeType.SENSE_ESF_VAL,
            NodeType.SENSE_INTERNAL_ENERGY,
        ]
        ...
        for node_id in topo_order:
            ...
            if node.node_type == NodeType.DELAY:
                input_edges = [e for e in self.edges if e["enabled"] and e["target_id"] == node_id]
                if input_edges:
                    # 计算当前输入
                    input_val = 0.0
            ...
            # 🚀 v17.0: MODULATOR 门控算子
            # v17.1 阶段一修复: 添加预偏置 (Pre-bias)
            # 核心逻辑: signal_sum * sigmoid(control_sum + bias)
            # - 信号边 (is_modulatory=False): 累积到 signal_sum
            # - 调节边 (is_modulatory=True): 累积到 control_sum
            # - bias: 预偏置使新节点默认"开启"，平滑过渡旧网络
                ...
                for edge in input_edges:
                    source_node = self.nodes[edge["source_id"]]
                    edge_val = source_node.activation * edge["weight"]

                    if edge.get("is_modulatory", False):
                        # 调节边: 控制门控强度
                        control_sum += edge_val
                    else:
                        # 信号边: 待调制的信号
                        signal_sum += edge_val

                # v17.1: 预偏置 - 从节点属性或config获取
                # 默认 bias=2.0 → sigmoid(2.0)≈0.88 (默认开启)
                bias = getattr(node, "modulator_bias", 2.0)

                # 门控计算: sigmoid(control + bias) 控制 signal 的通过量
                if abs(control_sum) > 1e-6 or bias != 0.0:
                    gate = 1.0 / (1.0 + np.exp(-(control_sum + bias)))  # sigmoid with bias
                else:
                    gate = 1.0  # 无调节信号且无偏置时，透传

                node.activation = signal_sum * gate
        ...
        actuator_nodes = [
            ...
            in (
                ...
                NodeType.ACTUATOR_PERMEABILITY,
                NodeType.ACTUATOR_THRUST_X,
                NodeType.ACTUATOR_THRUST_Y,
                NodeType.ACTUATOR_SIGNAL,
        ...
        # 🚀 v14.0: 简化输出 - 只保留执行器
        actuator_outputs = [
            apply_activation(n.node_type.name, n.activation) for n in actuator_nodes
        ]
    ...
    def mutate_add_node(self, output_weight: float = None) -> bool:
        """
        ...
        参数:
            output_weight: 新节点的输出权重
                - None 或 1.0: 当前方案 (保持信号强度)
                - 0.0: 零权重方案 (Net-2-Net, 无损插入)
                默认: None (保持向后兼容)

        返回:
            是否成功执行突变
        """
        # Net-2-Net: 默认使用1.0保持向后兼容
        ...
        if rand < 0.30:
            ...
            node_type = NodeType.MODULATOR  # v17.0: 门控算子 (15%概率)
        ...
        if node_type == NodeType.DELAY:
            ...
            new_node = Node(
                node_id=new_node_id,
                node_type=node_type,
                name=f"{node_type.name}_{new_node_id}",
                delay_steps=delay_steps,
            )
        else:
            new_node = Node(
                node_id=new_node_id, node_type=node_type, name=f"{node_type.name}_{new_node_id}"
            )
        ...
        # ============================================================
        # v17.1 阶段一修复: 带噪恒等初始化 (Noisy Identity Init)
        # 打破零权重死亡之谷: 零初始化+微小噪声→打破对称性，赋予探索能力
        # ============================================================
        # 配置参数 (可通过PoolConfig覆盖)
        noisy_init = getattr(self, "config", None)
        sigma = getattr(noisy_init, "NOISY_IDENTITY_SIGMA", 0.1) if noisy_init else 0.1
        use_noisy = getattr(noisy_init, "NOISY_IDENTITY_INIT", True) if noisy_init else True

        if use_noisy:
            # source → new_node: 1.0 + N(0, σ²) - 保持主信号，微小扰动
            w_input = 1.0 + np.random.randn() * sigma
            # new_node → target: output_weight + N(0, σ²) - 保留配置，微小扰动
            w_output = (
                output_weight + np.random.randn() * sigma
                if output_weight
                else np.random.randn() * sigma
            )
        else:
            # 传统方案: 恒等映射
            w_input = 1.0
            w_output = output_weight if output_weight else 1.0

        self.add_edge(edge["source_id"], new_node_id, weight=w_input, enabled=True)
        self.add_edge(new_node_id, edge["target_id"], weight=w_output, enabled=True)
    ...
    def mutate_add_edge(self, max_attempts: int = 50) -> bool:
        ...
        返回:
            是否成功执行突变
        """
        # 🚀 v17.0: 源节点类型 (包含 MODULATOR 作为输出源)
        source_nodes = [
            n.node_id
            for n in self.nodes.values()
            if n.node_type
        ...
        # 🚀 v17.0: 目标节点类型 (包含 MODULATOR 作为门控目标)
        target_nodes = [
            n.node_id
            for n in self.nodes.values()
            if n.node_type
        ...
        for _ in range(max_attempts):
            ...
            # 🚀 v17.0: 如果目标是 MODULATOR，50%概率设为调节边
            target_node = self.nodes.get(target_id)
            is_modulatory = (
                target_node
                and target_node.node_type == NodeType.MODULATOR
            ...
            # 设置调节边属性
            if is_modulatory:
                self.edges[-1]["is_modulatory"] = True
    ...
    def copy(self) -> "OperatorGenome":
        ...
        """
        new_genome = OperatorGenome(innovation_manager=self._innovation_mgr, config=self.config)
        ...
        for node in self.nodes.values():
            ...
            # 复制 SENSOR 相关属性
            if getattr(node, "angle_weights", None) is not None:
            ...
            # v17.1: 复制 MODULATOR 偏置
            if getattr(node, "modulator_bias", None) is not None:
                new_node.modulator_bias = node.modulator_bias
    ...
    def hebbian_update(
        self, node_activations: Dict[int, float], lr: float = 0.01, reward: float = 0.0
    ):
        """
        赫布规则更新可塑性边 (奖励调制版本)

        Hebbian rule: Δw = lr * reward * pre_activation * post_activation
        "Fire together, wire together, BUT only if it pays off"

        参数:
            node_activations: 节点激活值字典 {node_id: activation}
            lr: 学习率系数
            reward: 奖励信号 (能量变化差值)，可正可负
                - 正值: 增强激活的连接 (吃到能量)
                - 负值: 削弱激活的连接 (掉血/撞墙)
                - 零: 无监督Hebbian (传统模式)
        """
        # 审计修复: 隐患3 - 奖励调制的Hebbian学习
        # 将reward归一化到 [-1, 1] 范围，避免梯度爆炸
        reward_factor = np.clip(reward / 10.0, -1.0, 1.0)  # 假设10是典型能量变化
        ...
        for edge in self.edges:
            ...
            # 奖励调制Hebbian: Δw = lr * reward_factor * pre * post
            # reward > 0: 增强连接 (好的行为)
            # reward < 0: 削弱连接 (坏的行为)
            # reward = 0: 传统无监督Hebbian (向后兼容)
            delta = learning_rate * lr * reward_factor * pre_act * post_act
        ...
        new_edges = [
            {
                ...
                "target_id": supernode.input_ids[0]
                if supernode.input_ids
                else old_edge["target_id"],
            ...
            {
                "source_id": supernode.output_ids[0]
                if supernode.output_ids
                else old_edge["source_id"],
    ...
    def from_dict(cls, data: Dict) -> "OperatorGenome":
        ...
        for node_data in data["nodes"]:
            ...
            node = node_from_dict(node_data, Node)
    ...
    def mutate(self, rate: float = 0.1, add_node_output_weight: float = None) -> "OperatorGenome":
        """
        统一变异接口 - 用于分裂时的基因突变
        ...
        Args:
            rate: 变异概率
            add_node_output_weight: 新节点的输出权重 (Net-2-Net)
                - None: 使用默认行为 (1.0)
                - 0.0: 零权重方案 (无损插入)
        ...
        if random.random() < rate * 0.3:
            # 添加节点 (传入输出权重)
            child.mutate_add_node(output_weight=add_node_output_weight)
    ...
    def apply_baldwin_assimilation(
        self, parent_genome: "OperatorGenome", kappa: float = 0.5, sigma: float = 0.01
    ) -> "OperatorGenome":
        """
        深度鲍德温遗传同化
