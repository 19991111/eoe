...
class DynamicEnergySource(EnergySource):
    """
    v17.3 动态能量源 - 支持莱维飞行/歇息-爆发机制
    
    继承自EnergySource，添加动态移动能力
    即插即用: USE_DYNAMIC_ENERGY_SOURCE=False时等效于静态源
    """
    ...
    def __init__(
        ...
        jump_prob: float = 0.2,
        jump_dist: float = 5.0,
        wiggle_radius: float = 1.0
    ):
        super().__init__(x, y, injection_rate, radius)
    ...
    def should_move(self, current_step: int) -> bool:
        """判断当前步是否应该移动"""
        self._steps_since_move += 1
        if self._steps_since_move >= self.move_interval:
            self._steps_since_move = 0
            return True
        return False
    
    def move(self, width: float, height: float) -> bool:
        """
        执行莱维飞行移动
        返回: True=发生了位移, False=保持静止
        """
        import random
        
        if random.random() < self.jump_prob:
            # 跳跃模式: 随机方向移动jump_dist
            angle = random.uniform(0, 2 * 3.14159)
            dx = self.jump_dist * (random.random() * 0.5 + 0.5)  # 0.5-1.0倍jump_dist
            new_x = (self.position[0] + dx * math.cos(angle)) % width
            new_y = (self.position[1] + dx * math.sin(angle)) % height
            self.position = (new_x, new_y)
        else:
            # 蠕动模式: 小范围随机移动
            dx = random.uniform(-self.wiggle_radius, self.wiggle_radius)
            dy = random.uniform(-self.wiggle_radius, self.wiggle_radius)
            new_x = (self.position[0] + dx) % width
            new_y = (self.position[1] + dy) % height
            self.position = (new_x, new_y)
        
        # 检查是否真的移动了
        dist = math.sqrt((self.position[0] - self._last_position[0])**2 + 
                        (self.position[1] - self._last_position[1])**2)
        self._last_position = self.position
        return dist > 0.1
    
    def __repr__(self):
        return f"DynamicEnergySource{self.position} rate={self.injection_rate:.2f} r={self.radius}"
...
class EnergyField:
    ...
    def _clear_energy_at(self, x: float, y: float, radius: float):
        """
        v17.3: 清除指定位置的能量场 (动态源移动后调用)
        将指定区域的能量衰减至背景值
        """
        x_idx = int(x / self.resolution)
        y_idx = int(y / self.resolution)
        radius_idx = int(radius / self.resolution)
        
        x_range = range(max(0, x_idx - radius_idx), min(self.grid_width, x_idx + radius_idx + 1))
        y_range = range(max(0, y_idx - radius_idx), min(self.grid_height, y_idx + radius_idx + 1))
        
        for ix in x_range:
            for iy in y_range:
                if 0 <= ix < self.grid_width and 0 <= iy < self.grid_height:
                    # 衰减至背景值
                    self.field[ix, iy] = min(self.field[ix, iy], self.field_initial_energy)
...
class Environment:
        ...
        for other in self.agents:
            ...
            if dist < collision_dist:
                ...
                if my_force > other_force and other.internal_energy > 10:
                    ...
                    # 跳过无限能量的Agent
                    if not np.isinf(other.internal_energy):
                        other.internal_energy -= steal_amount
                    
                    recoil_loss = steal_amount * 0.10  # 10%反作用力
                    # 跳过无限能量的Agent
                    if not np.isinf(agent.internal_energy):
    ...
    def step(self) -> None:
        ...
        # ============================================================
        # v17.3: 动态能量源更新 (追猎动力学)
        # 即插即用: 仅当source是DynamicEnergySource时生效
        # ============================================================
        if self.energy_field_enabled and self.energy_field:
            for source in self.energy_field.sources:
                if isinstance(source, DynamicEnergySource):
                    if source.should_move(self.step_count):
                        # 记录旧位置用于清除旧场
                        old_x, old_y = source.position
                        
                        # 执行移动
                        source.move(self.width, self.height)
                        
                        # 清除旧位置的能量场 (衰减旧场)
                        self._clear_energy_at(old_x, old_y, source.radius)
        ...
        for agent in self.agents:
            ...
            # 2. 计算 Surprise (预测误差)
            # 检查形状兼容性: sensor_values可能有多维传感器
            if (agent.predicted_next_sensor is not None and 
                agent.last_sensor_inputs is not None and
                len(agent.predicted_next_sensor) == len(sensor_values)):
            ...
            # 修复: 如果只有1个输出，复制为两个（左右轮都需要）
            # 否则agent会完全不动
            if len(actuator_outputs) == 1:
                actuator_outputs = np.array([actuator_outputs[0], actuator_outputs[0]])
            elif len(actuator_outputs) < 2:
            ...
            # 动作越强烈，能量消耗越多 ( 追捕需要消耗更多能量)
            action_intensity = np.mean(np.abs(actuator_outputs))
            metabolic_cost += action_intensity * 0.5  # 提高10倍 - 追捕昂贵
            ...
            # ============================================================
            # 跳过无限能量 (FrozenAgent测试模式)
            if (self.energy_decay_k > 0 and agent.internal_energy > 0 
                and not np.isinf(agent.internal_energy)):
            ...
            # 审计修复: 隐患3 - 奖励调制Hebbian学习
            # 在步骤开始时记录能量（这样才能捕捉到整步的代谢变化）
            if not hasattr(agent, '_energy_at_step_start'):
                agent._energy_at_step_start = agent.internal_energy
            ...
            # 计算这一帧的energy delta (整步的代谢变化)
            energy_delta = agent.internal_energy - agent._energy_at_step_start
            agent.genome.hebbian_update(agent_activations, lr=0.01, reward=energy_delta)
            
            # 更新能量记录供下一步使用
            agent._energy_at_step_start = agent.internal_energy
            
            # 记录供下一帧参考
            agent._last_energy_delta = energy_delta
