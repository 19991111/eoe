...
class SubgraphMiner:
    ...
    def mine(
        ...
        # v16.16 修复: 引入拓扑结构的模式匹配 (方案1+4)
        # 3. 统计频率 - 现在考虑"节点类型 + 排序后的边连接"
        pattern_counts: Dict[Tuple, int] = defaultdict(int)
        pattern_agents: Dict[Tuple, List[int]] = defaultdict(list)
        pattern_node_types: Dict[Tuple, Tuple[int, ...]] = {}
        pattern_adjacency: Dict[Tuple, Any] = {}

        for agent_idx, encoding in encoded_brains:
            node_types, edge_types, adjacency = encoding
            
            # v16.16: 使用图签名 = 节点类型 + 排序边
            # 边需要按节点ID排序，保证同构子图得到相同签名
            sorted_edges = tuple(sorted(zip(edge_sources, edge_targets)))
            graph_signature = (tuple(node_types), sorted_edges)
            
            # 只统计长度在min_size到max_size之间的
            if self.min_size <= len(node_types) <= self.max_size:
                pattern_counts[graph_signature] += 1
                pattern_agents[graph_signature].append(agent_idx)
                # 保存用于创建pattern
                pattern_node_types[graph_signature] = tuple(node_types)
                pattern_adjacency[graph_signature] = adjacency

        # 4. 转换为SubgraphPattern (使用图签名)
        frequent_patterns = []
        
        # v16.16 修复: 使用动态阈值 (方案2)
        # 候选池大小的20%，保底2个
        candidate_pool_size = len(encoded_brains)
        min_count = max(2, int(candidate_pool_size * 0.2))
        
        for graph_signature, count in pattern_counts.items():
            if count >= min_count:
                node_types = pattern_node_types[graph_signature]
                adjacency = pattern_adjacency[graph_signature]
                
                # 从签名中提取边信息
                _, sorted_edges = graph_signature
                
                pattern = SubgraphPattern(
                    node_types=node_types,
                    edge_types=tuple([1] * len(sorted_edges)),  # 用边数作为edge_types
                    adjacency=adjacency,
                    support=count
                )
                pattern.agent_indices = pattern_agents[graph_signature]
