...
from .node import Node, NodeType, SuperNode, MacroOperator, SubgraphTracker
from .genome import OperatorGenome
from .agent import Agent  # 需要在运行时导入，因为需要实例化Agent
from .environment import Environment, ChunkManager  # 需要在运行时导入，因为需要实例化
...
class SubgraphFreezer:
    ...
    def extract_subgraphs(
        self, elite_agents: List["Agent"], generation: int, avg_energy: float = 0.0
    ) -> List[MacroOperator]:
        """
        从精英智能体中提取子图并冻结
        ...
        # Step 3: 筛选高频模式
        frequent_patterns = {p: c for p, c in topology_freq.items() if c >= self.min_frequency}
        ...
        for agent in elite_agents:
            ...
            internal_nodes = [nid for nid, n in nodes.items() if n.node_type in internal_types]

            # 从每个内部节点开始搜索
            for start_id in internal_nodes:
                subgraph_nodes = self._find_connected_nodes(nodes, edges, start_id, depth=2)

                if len(subgraph_nodes) >= self.min_connections:
                    # 创建MacroOperator
                    macro = self._create_macro(nodes, edges, subgraph_nodes, generation)
                    if macro:
    ...
    def _extract_topology_pattern(self, genome: "OperatorGenome") -> List[Tuple]:
        ...
        for edge in genome.edges:
            ...
            has_residual = any(
                e["enabled"]
                and e["source_id"] == edge["target_id"]
    ...
    def _find_connected_nodes(
        self, nodes: Dict[int, Node], edges: List[Dict], start_id: int, depth: int = 2
    ) -> Dict[int, Node]:
    ...
    def _make_pattern(
        self, nodes: Dict[int, Node], edges: List[Dict], subgraph: Dict[int, Node]
    ) -> Tuple:
        """创建子图的模式签名"""
        node_types = tuple(nodes[nid].node_type.name for nid in sorted(subgraph.keys()))
    ...
    def _create_macro(
        self, nodes: Dict[int, Node], edges: List[Dict], subgraph: Dict[int, Node], generation: int
        ...
        # 创建MacroOperator
        node_list = [
            {"id": nid, "type": n.node_type.name, "name": n.name} for nid, n in subgraph.items()
        ]
...
class MultiLevelPredictor:
    ...
    def select_targets(self, agent: "Agent") -> List[int]:
        ...
        candidates = [
            nid
            for nid, node in agent.genome.nodes.items()
            if node.node_type in self.target_types
            and node.node_type not in [NodeType.SENSOR, NodeType.ACTUATOR, NodeType.CONSTANT]
        ]
    ...
    def predict(self, agent: "Agent", current_activations: Dict[int, float]) -> Dict[int, float]:
        """
        预测下一时刻的节点状态
    ...
    def calculate_error(self, agent: "Agent", actual_activations: Dict[int, float]) -> float:
        """计算预测误差"""
...
class Population:
    ...
    def __init__(
        ...
        self.subgraph_freezer = SubgraphFreezer(
            freeze_interval=20, top_percent=0.05, min_connections=3, max_macros=10
        )
        ...
        self.spontaneous_activity = SpontaneousActivity(
            threshold=0.1, noise_scale=0.05, enable_prob=0.3
        )

        # 多级预测驱动
        self.multi_predictor = MultiLevelPredictor(num_targets=2, prediction_weight=2.0)
    ...
    def select_true_champion(self, agents: List["Agent"]) -> "Agent":
        ...
        if self.generation % 10 == 0:
            print(
                f"  [熔炉] 真冠军: fit={best['true_fitness']:.1f} "
    ...
    def _calculate_environmental_pressure(self) -> float:
        ...
        if self.environment.n_food > 0:
            food_density = self.environment.n_food / (
        ...
        if self.environment.seasonal_cycle:
            ...
            # 季节长度不确定性
            pressure += (
                self.environment.season_jitter * 0.2
                if hasattr(self.environment, "season_jitter")
                else 0
            )
    ...
    def _calculate_complexity_score(self, agent: "Agent") -> float:
        ...
        brain_cost = (
            total_nodes * self.brain_metabolic_alpha + total_edges * self.brain_metabolic_beta
        )
        ...
        meta_nodes = sum(1 for n in genome.nodes.values() if n.node_type.name == "META_NODE")
        predictor_nodes = sum(1 for n in genome.nodes.values() if n.node_type.name == "PREDICTOR")
        delay_nodes = sum(1 for n in genome.nodes.values() if n.node_type.name == "DELAY")
    ...
    def update_hall_of_fame(self, champion: "Agent") -> bool:
        ...
        if current_strength > best_history["fitness"] * 1.2:
            # 替换最差的
            worst_idx = min(
                range(len(self.hall_of_fame)), key=lambda i: self.hall_of_fame[i]["fitness"]
            )
    ...
    def epoch(self, verbose: bool = False) -> Dict:
        ...
        for step in range(self.lifespan):
            ...
            for agent in self.agents:
                ...
                # 计算预测误差
                if step > 0:
                    pred_error = self.multi_predictor.calculate_error(agent, current_activations)
            ...
            if verbose and step % 100 == 0:
                ...
                print(
                    f"  Generation {self.generation}, Step {step}: "
        ...
        for agent in self.agents:
            ...
            if agent.is_alive and agent.internal_energy > 0:
                agent.fitness = (
                    agent.fitness + novelty * novelty_weight + exploration * exploration_weight
                )
    ...
    def reproduce(self, verbose: bool = False) -> None:
        ...
        # 按能量降序排序，确保选择能量更高的父母
        survivors_with_energy = sorted(
            survivors_with_energy, key=lambda a: a.internal_energy, reverse=True
        )
        ...
        def can_reproduce(agent: Agent) -> bool:
            ...
            energy_pct = (
                (agent.internal_energy / agent.initial_energy) * 100
                if agent.initial_energy > 0
                else 0
            )
        ...
        for i in range(self.population_size):
            ...
            if i < n_elite_persistent and i < len(persistent_genomes):
                ...
                i < n_elite_persistent + n_micro_mutation
                and len(micro_mutation_genomes) > 0
                and (i - n_elite_persistent) < len(micro_mutation_genomes)
            ):
                # Tier 2: 微突变 - 仅权重小扰动
        ...
        # ============================================================
        if (
            hasattr(self.environment, "stigmergy_field_enabled")
            and self.environment.stigmergy_field_enabled
        ):
    ...
    def _check_operator_extinction(self, verbose: bool = False) -> None:
        ...
        for macro in pool:
            if macro.should_extinct(self.generation):
                ...
                if verbose:
                    print(
                        f"  [Extinction] 移除 MacroOperator #{macro.id} "
        ...
        if extinct_count > 0:
            ...
            if verbose:
                print(
                    f"  [Extinction] 算子池: {len(pool)} -> {len(surviving)} (移除{extinct_count}个)"
                )

    def run(self, n_generations: int = 10, verbose: bool = True) -> List[Dict]:
        """
        运行多代演化
        ...
        for gen in range(n_generations):
            ...
            if self.subgraph_freezer.should_freeze(self.generation + 1):
                ...
                new_macros = self.subgraph_freezer.extract_subgraphs(
                    elite_agents, self.generation + 1, avg_energy=avg_energy
                )
...
def demo():
    ...
    print(
        f"距目标距离: {np.sqrt((env.target_pos[0] - agent.x) ** 2 + (env.target_pos[1] - agent.y) ** 2):.2f}"
    )
...
try:
    ...
    print(
        "Note: matplotlib not available. Install with 'pip install matplotlib' for visual brain plots."
    )
...
def print_brain_text(agent: Agent) -> str:
    ...
    for node in sorted(genome.nodes.values(), key=lambda n: n.node_id):
        if node.node_type == NodeType.DELAY:
            lines.append(
                f"  #{node.node_id}: {node.node_type.name:10s} (act={node.activation:7.3f}, delay_state={node.delay_state:7.3f})"
            )
        else:
            lines.append(
                f"  #{node.node_id}: {node.node_type.name:10s} (act={node.activation:7.3f})"
            )
    ...
    for edge in genome.edges:
        ...
        if src and tgt:
            lines.append(
                f"  #{edge['source_id']}({src.node_type.name:10s}) --> #{edge['target_id']}({tgt.node_type.name:10s})  [w={edge['weight']:+.3f}]"
            )
...
def visualize_brain(
    agent: Agent, ax=None, figsize: Tuple[int, int] = (10, 8), title: Optional[str] = None
):
    """
    可视化智能体的大脑拓扑图
    ...
    for edge in edges:
        ...
        ax.annotate(
            ...
            xytext=start,
            arrowprops=dict(
                arrowstyle="->", color=color, lw=linewidth, connectionstyle="arc3,rad=0.1"
            ),
        )
    ...
    for node_id, (x, y) in positions.items():
        ...
        ax.scatter(
            x, y, s=size, c=color, marker=marker, edgecolors="black", linewidths=2, zorder=10
        )

        # 添加标签
        ax.text(x, y, label, ha="center", va="center", fontsize=10, fontweight="bold", zorder=20)

        # 显示节点 ID
        ax.text(x, y - 0.3, f"#{node_id}", ha="center", va="top", fontsize=8, color="gray")
...
def visualize_population_best(population: Population, generation: int = None):
    ...
    visualize_brain(best_agent, ax=ax, title=f"Generation {gen_num} - Best Agent Brain")
...
def demo_evolution():
    ...
    for i, stats in enumerate(history):
        print(
            f"Generation {i}: best={stats['best_fitness']:.2f}, "
