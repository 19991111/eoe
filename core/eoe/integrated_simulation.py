...
class IntegratedSimulation:
    ...
    def __init__(
        ...
        # 能量守恒检查频率 (优化: 不每步检查)
        self._energy_check_interval = 100
        self._last_energy_check = 0
    ...
    def step(self, verbose: bool = False) -> SimState:
        ...
        if m.energy_extraction and self.step_count - self._last_energy_check >= self._energy_check_interval:
            ...
            self._last_energy_check = self.step_count
    ...
    def run(
        ...
        for step in range(max_steps):
            ...
            # 早停: 如果全部死亡
            alive_count = torch.sum(self.alive_mask).item()
            
            # 优化: 存活率<1%且已运行>100步时提前结束
            if alive_count == 0:
                if verbose:
                    print(f"  ⚠️ 全部Agent死亡于 step {step}")
                break
            elif step > 100 and alive_count / self.n_agents < 0.01:
                if verbose:
                    print(f"  ⚠️ 存活率过低 ({alive_count}/{self.n_agents}), 提前结束于 step {step}")
                break
