...
class PhysicsManifest:
    ...
    predation_range: float = 6.0          # 捕食范围 (增强: 4.0 → 6.0)
    predation_rate: float = 1.5           # 吸血效率 (增强: 0.8 → 1.5)
    predation_cost: float = 0.05          # 捕食代谢成本
    predation_mutation: float = 0.25      # 产生捕食者的突变概率 (增强: 0.15 → 0.25)
    ...
    # ==================== GPU Stigmergy Field (v16.23) ====================
    # 启用GPU加速的Stigmergy信息素场
    # - Agent移动时自动留下"气味"
    # - 捕食攻击时释放"警报信息素"
    # - 捕食者可以追踪气味，猎物可以躲避危险区域
    stigmergy_field_enabled: bool = False     # GPU信息素场开关
    stigmergy_field_deposit_amount: float = 0.02  # 移动时沉积量
    stigmergy_field_alarm_amount: float = 0.5     # 捕食时警报量
    stigmergy_field_diffusion: float = 0.05       # 扩散率
    stigmergy_field_decay: float = 0.99           # 衰减率
    ...
    n_rivals: int = 10              # 增强: 3 → 10
    rival_refresh_interval: int = 20  # 增强: 40 → 20
