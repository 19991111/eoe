...
class SuperNodeRegistry:
    ...
    def _match_pattern(
        ...
        # 3. 边结构匹配 (核心修复!)
        # 提取输入边的排序列表
        input_sorted = tuple(sorted(edges))
        spec_sorted = tuple(sorted(spec.connections))
        
        if input_sorted != spec_sorted:
            # 尝试更宽松的匹配：检查边是否"包含"spec的连接
            # 允许SuperNode作为更大图的子结构
            if not all(c in input_sorted for c in spec_sorted):
                return False
