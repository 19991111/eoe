...
print("=" * 60)
print("v0.97 Stage 1: 降维抢救 - 只开疲劳机制")
print("  目标: 让Agent形成'累了就歇'的本能")
print("  - 只保留: 疲劳减速 + 静止恢复")
print("  - 注释掉: 起床饥饿、睡眠掉落、信息素、食物腐败")
print("  - 新增: 睡眠成功奖励 +10")
print("=" * 60)
...
# 方案1: 代谢疲劳 + 安全掩体 (Stage 1纯净版)
# 关键: 禁用起床饥饿和物理掉落，让Agent先学会"累了就歇"
pop.environment.enable_fatigue_system(
    ...
    enable_wakeup_hunger=False, # Stage 1: 禁用起床饥饿
    enable_sleep_drop=False     # Stage 1: 禁用物理掉落
)
...
for gen in range(500):  # 延长到500代，让行为耦合稳定下来
    # 重置食物
    ...
    for a in pop.agents:
        ...
        # Stage 1: 睡眠奖励 (让"累了就歇"有利可图)
        sleep_bonus = a.sleep_cycles * 15  # 每次睡眠尝试 +15
