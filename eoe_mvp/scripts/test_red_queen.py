...
10000步 - 红皇后假说强化版
==========================
使用 PhysicsManifest 配置，开启真正的红皇后军备竞赛
"""
...
def run():
    ...
    print(f"🔧 红皇后配置: range={PREDATION_RANGE}, rate={PREDATION_RATE}, mutation={PREDATION_MUTATION}")
    print(f"{'='*60}")
    print(f"🧪 10000步 - 红皇后假说强化版")
    print(f"{'='*60}")
    ...
    for step in range(n_steps):
        ...
        if batch.n > 0:
            ...
            # 红皇后机制: 根据配置的概率产生捕食者
            attack_prob = PREDATION_MUTATION
            attack_mask = torch.rand(batch.n, device=device) < attack_prob
    ...
    ax.set_title('Population (Boom-Bust Cycles)')
    ...
    ax.set_title('Brain Complexity (Node Count)')
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title('Node Distribution Heatmap')
    ax.set_yticks(range(9))
    ax.set_yticklabels(range(1, 10))
    plt.colorbar(im, ax=ax)
    
    plt.tight_layout()
    plt.savefig('evolution_red_queen.png', dpi=150)
    print(f"✅ 图表已保存: evolution_red_queen.png")
