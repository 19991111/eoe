...
def run():
    ...
    ax.set_xlabel('Step')
    ax.set_ylabel('Count')
    ax.set_title('Brain Complexity (Long-term Evolution)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ax.axhline(y=8, color='red', linestyle='--', alpha=0.5, label='8 nodes')
    ax.axhline(y=9, color='purple', linestyle='--', alpha=0.5, label='9 nodes')
    ax.axhline(y=10, color='black', linestyle='--', alpha=0.5, label='10 nodes')
    ...
    ax.set_yticklabels(range(6, 15))
    plt.colorbar(im, ax=ax)
    
    plt.tight_layout()
    plt.savefig('evolution_supernode_10k.png', dpi=150)
    print(f"✅ 图表已保存: evolution_supernode_10k.png")
    
    print(f"\n{'='*60}")
    print("🏆 最终结果 - 10000步长时间演化")
    print("="*60)
    ...
    if batch.n > 0:
        ...
        for sn in range(0, 8):
            count = (agents.state.supernodes[batch.indices] == sn).sum().item()
            if count > 0:
                print(f"  Supernodes {sn}: {count} ({100*count/batch.n:.1f}%)")
