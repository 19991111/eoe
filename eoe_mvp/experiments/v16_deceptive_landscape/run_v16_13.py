...
def run_v16_13_experiment(
    steps: int = 6000,
    initial_pop: int = 50,
    device: str = 'cuda:0'
):
    print("=" * 70)
    print("🔬 v16.13 欺骗性景观实验 - 课程学习终极版")
    print("=" * 70)
    print("  目标: 移除人工税收，用环境压力自然筛选")
    print("  策略: 认知溢价 + 课程学习三阶段")
    print("  阶段I: 80%可见(伊甸园) | 阶段II: 40%可见(筛选) | 阶段III: 10%可见(终极)")
    print("  核心: 移除定居者税，让演化自然选择")
    print("=" * 70)
    ...
    # 不再使用定居者税，用环境难度自然筛选
    CURRICULUM_PHASES = [
        (0, 1000, 0.8, "🌱 伊甸园: 80%可见"),      # 宽松环境，让种群繁荣
        (1000, 3000, 0.4, "🌊 大筛选: 40%可见"),   # 中等难度，筛选预测能力
        (3000, 6000, 0.15, "🔥 终极: 15%可见"),   # 地狱难度，稳定高智商个体
    ]
    current_phase = 0
    ...
    for step in range(steps):
        ...
        if (step + 1) % 200 == 0:
            ...
            print(f"  Step {step+1:5d} | 存活: {n_alive:3d} | "
                  ...
                  f"速度: {avg_vel:.3f} | {'可见' if is_visible else '隐身'} | {elapsed:.1f}s")
...
if __name__ == "__main__":
    ...
    run_v16_13_experiment(args.steps, args.pop, args.device)