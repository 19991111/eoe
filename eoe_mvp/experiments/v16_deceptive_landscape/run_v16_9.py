...
class StationaryTaxModifier:
    ...
    直接修改agents.state.energies中的能量
    """
    
    def __init__(
        self,
        threshold_velocity: float = 0.1,  # 速度低于此值视为"静止"
        tax_steps: int = 50,              # 连续静止多少步后开始征税
        tax_rate: float = 0.05,           # 每步额外消耗
    ):
    ...
    def apply(self, agents):
        ...
        # 获取所有Agent的速度和存活状态
        all_speeds = agents.state.linear_velocity  # [max_agents]
        alive_mask = agents.alive_mask  # [max_agents]
        
        # 低于阈值 = 静止
        is_stationary = all_speeds < self.threshold_velocity
        
        # 更新静止计数 (所有Agent)
        self.stationary_counter = torch.where(
            is_stationary,
            self.stationary_counter + 1,
            torch.zeros_like(self.stationary_counter)  # 移动则重置
        )
...
def run_v16_9_experiment(
    steps: int = 6000,
    initial_pop: int = 50,
    device: str = 'cuda:0'
):
    print("=" * 70)
    print("🔬 v16.9 欺骗性景观实验 - 定居者税版")
    print("=" * 70)
    print("  目标: 打破伏地魔策略")
    print("  策略: 静止税 + 可预测轨迹 + 课程学习")
    print("=" * 70)
    ...
    stationary_tax = StationaryTaxModifier(
        threshold_velocity=0.1,
        tax_steps=50,      # 连续50步静止后开始征税
        tax_rate=0.08,     # 每步额外消耗0.08能量
    )
    ...
    agents = BatchedAgents(
        ...
        env_height=100.0,
        device=device,
        init_energy=80.0,   # 降低初始能量，强制早期移动
        config=config,
        env=env
    )
    ...
    results = {
        'version': 'v16.9',
        'config': {
            'stationary_tax_threshold': stationary_tax.threshold_velocity,
...
if __name__ == "__main__":
    ...
    run_v16_9_experiment(args.steps, args.pop, args.device)